...
#Sharir seuratobject
pool.data<- ReadMtx(
  mtx = "/Users/shibatashouta/Desktop/a large pool data/GSM3767568control/GSM3767568_control_matrix.mtx", #GSM3767568_control_matrix.mtx.gz
  features = "/Users/shibatashouta/Desktop/a large pool data/GSM3767568control/GSM3767568_control_genes.tsv", #GSM3767568_control_genes.tsv.gz
  cells = "/Users/shibatashouta/Desktop/a large pool data/GSM3767568control/GSM3767568_control_barcodes.tsv" #GSM3767568_control_barcodes.tsv.gz
)


pool.meta <- read.table("/Users/shibatashouta/Desktop/a large pool data/GSE131204/GSE131204_cell_info_8594x25.tsv", #GSE131204_cell_info_8594x25.tsv.gz
                        sep = "\t", header = TRUE)
...
atlas1.meta = read.table("/Users/shibatashouta/Desktop/dental cell type atlas/annotation_mouse_incisor.txt",
                         header = FALSE, sep = "\t")
...
saveRDS(dat1.combined, "/Users/shibatashouta/Desktop/dental cell type atlas/pool_atlas_combined")# after integration data
...
====================================================================================-
  #細胞名が明瞭なclusterに名前を与えていく Labeling from simple clusters.
...
#Since cluster14 was an unnamed cell population, it was removed as it could not be determined at this time.
==========================================================================================================================
  #Immune cell 系
...
#pulp3と５は名無しが多かったり細胞種に統一性が見られないためノイズとみなした
#pulp3 and 5 were considered as noise because there were many nameless cells
...
seuratObj = dat1
#remove cells whose idents = Ameloblasts.it is because these cells do not have any gene features
#, which codes enamel matrix protein and I regard these cells as noise
# but Amelotin is highly expressed in these cell populations. Therefore, it is possible that they are mature ameloblasts
#, so it is best to skip this step if you want to use them in your analysis.
#However, it is unnatural for only mature enamel blast cells to congregate.
...
dat1$celltype%>%table()
#Identify which cell types are localized in the Near cervical area.
...
#Other genes with known expression were added with reference to https://bite-it.helsinki.fi 
...
orignal_geneset
#type1 → not selective
#type2 →Extract only those genes known to cause some dental damage due to the defect. geneset_GO
#type3 →geneset GO + Genes that have been found to be expressed in oral tissues. original_geneset
...
Idents(dat1)%>%unique()
#----------------------------------------
#type1  geneset_dental_formation
